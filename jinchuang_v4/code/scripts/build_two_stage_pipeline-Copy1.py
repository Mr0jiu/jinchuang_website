...
def build_stage1_visual_relation_model(random_state: int) -> Pipeline:
    """Lightweight image-only classifier for direct reuse vs changed-scene renewal."""
    preprocess = ColumnTransformer(
        [
            (
                "num",
...
def add_stage1_visual_relation(pairs: pd.DataFrame, random_state: int) -> tuple[pd.DataFrame, dict[str, object]]:
    """Classify direct reuse vs changed-scene renewal in Stage 1 using images only.

    Explicit edit_type/base_from metadata remains authoritative. When those
    fields are empty, a small supervised model learns the observed distinction:
    F = copied/lightly transformed image, C/T = changed hair/clothes/background.
    """
    frame = pairs.copy()
    ...
    labeled = frame[renewal_label ^ repeat_label].copy()
    labeled["visual_renewal_label"] = renewal_label.loc[labeled.index].astype(int)

    frame["stage1_renewal_visual_probability"] = 0.0
    summary: dict[str, object] = {
        "method": "metadata_only",
        "training_rows": int(len(labeled)),
        "training_class_counts": dict(Counter(labeled["visual_renewal_label"])) if not labeled.empty else {},
        "threshold": RENEWAL_VISUAL_THRESHOLD,
    }
    if len(labeled) >= 20 and labeled["visual_renewal_label"].nunique() == 2:
        model = build_stage1_visual_relation_model(random_state)
        model.fit(labeled[IMAGE_FEATURE_COLUMNS], labeled["visual_renewal_label"])
        frame["stage1_renewal_visual_probability"] = model.predict_proba(frame[IMAGE_FEATURE_COLUMNS])[:, 1]
        summary["method"] = "logistic_regression_image_only"

    frame["stage1_visual_relation"] = np.where(
        frame["stage1_renewal_visual_probability"].ge(RENEWAL_VISUAL_THRESHOLD),
        "changed_hair_clothes_background",
        "direct_copy_or_light_manipulation",
    )
    frame["stage1_visual_renewal_pair"] = frame["stage1_visual_relation"].eq("changed_hair_clothes_background")

    # Stage 2 does not reclassify the image. It only combines the Stage-1
    # visual relation with customer identity/business metadata.
    ...
    frame["renewal_visual_pair"] = (
        ...
        & frame["stage1_visual_renewal_pair"]
    )
    ...
    summary["stage1_changed_scene_pairs"] = int(frame["stage1_visual_renewal_pair"].sum())
    summary["stage2_same_customer_renewal_pairs"] = int(frame["renewal_visual_pair"].sum())
...
def explain_stage2(row: pd.Series) -> str:
    ...
    if bool(row["renewal_effective_pair"]):
...
def main() -> None:
    ...
    pairs, stage1_visual_relation_summary = add_stage1_visual_relation(pairs, args.random_state)
    ...
    stage1_columns = [
        ...
        *IMAGE_FEATURE_COLUMNS,
        ...
        "stage1_label",
        "stage1_visual_relation",
        "stage1_renewal_visual_probability",
        "stage1_visual_renewal_pair",
        "same_similar_group",
        "query_similar_group",
        "match_similar_group",
        "query_path",
    ...
    summary = {
        "stage1": {
            ...
            "reuse_vs_renewal_visual_classifier": stage1_visual_relation_summary,
        ...
        "stage2": {
            ...
            "uses_stage1_visual_relation": True,
